odoo.define('Refund.list_template', function (require) {
"use strict";
console.log('refund tt')
    ...
    var ExpensesListController = ListController.extend(DocumentUploadMixin, {
        buttons_template: 'RefundListView.buttons',
    ...
    const ExpenseDashboardMixin = {
        _render: async function () {
            ...
            const result = await this._rpc({
                model: 'student.refund',
                method: 'get_refund_dashboard',
                context: this.context,
            });

            self.$el.parent().find('.o_expense_container').remove();
            const elem = QWeb.render('Refund.dashboard_refund_list_header', {
                expenses: result,
    ...
    viewRegistry.add('hr_refund_tree_dashboard_upload', ExpensesListViewDashboardUpload);
